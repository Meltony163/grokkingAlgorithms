...
void main()
{
	struct hash_table *my_hash_table = init_hash_table (10);
		...
		if( !find(my_hash_table, "tom", &my_entry) )
		    {
		        printf("let them vote!\n");
                assert(!insert(my_hash_table, "tom", "key"));
		    }
	   else
		    printf("kick them out!\n");

		if( !find(my_hash_table, "mike", &my_entry) )
        {
            printf("let them vote!\n");
            assert(!insert(my_hash_table, "mike", "key"));
        }
	   else
		    printf("kick them out!\n");
